...
import { RAYCAST_RENDERER_CONFIG, type RaycastRendererConfig } from "./RaycastRendererConfig";
...
interface RaycastWallTextureSample {
  width: number;
  height: number;
...
export class RaycastRenderer {
  ...
  private readonly wallTextureSamples = new Map<
    string,
    RaycastWallTextureSample | null
  >();
  ...
  constructor(
    ...
    private readonly config: RaycastRendererConfig = RAYCAST_RENDERER_CONFIG,
  ) {
    this.scene = scene;
  ...
  render(
    ...
    width: number,
    height: number,
    atmosphere: RaycastAtmosphereRenderOptions = getAtmosphereForDirector(
      null,
      0,
    ),
  ): void {
    ...
    for (let column = 0; column < this.config.rayCount; column += 1) {
      ...
      const wallHeight = Math.min(
        this.config.maxWallHeight,
        height / hit.correctedDistance,
      );
      const sectorShade =
        RAYCAST_ATMOSPHERE.sectorDarkness[
          hit.wallType as keyof typeof RAYCAST_ATMOSPHERE.sectorDarkness
        ] ?? 1;
      const surface = sampleRaycastSurfaceContext(
        this.level.zones,
        hit.hitX,
        hit.hitY,
        hit.rayAngle,
      ...
      const shade = Phaser.Math.Clamp(
        calculateFogShade(hit.correctedDistance, atmosphere) *
          sectorShade *
          (0.92 + surface.variant * 0.12) +
      ...
      const aoLikeShade = Phaser.Math.Clamp(
        shade * edgeDarken + columnNoise,
        atmosphere.ambientDarkness,
        1,
      );
      ...
      const color = this.blendColors(
        this.applyShade(baseWallColor, aoLikeShade),
        atmosphere.fogColor,
        1 - aoLikeShade,
      );
      ...
      const wallColumnWidth = Math.ceil(columnWidth) + 1;

      const didDrawWallTexture = this.drawTexturedWallColumn(
        hit,
        x,
        y,
        wallColumnWidth,
  ...
  renderEnemies(
    ...
    for (let i = 0; i < enemies.length; i += 1) {
      ...
      if (!enemy.alive && time > enemy.deathBurstUntil + RAYCAST_CORPSE_FADE_MS)
        continue;
      if (
        this.fillEnemyProjection(
          player,
          enemy,
          width,
          height,
          this.ensureEnemyProjectionSlot(n),
        )
      ) {
        n += 1;
      }
    ...
    for (let j = 0; j < list.length; j += 1) {
      ...
      const column = Phaser.Math.Clamp(
        Math.floor(projection.screenX / columnScale),
        0,
        this.config.rayCount - 1,
      );
      if (
        projection.distance >
        (this.depthBuffer[column] ?? Number.POSITIVE_INFINITY) + 0.05
      )
        continue;

      if (!projection.enemy.alive) {
        const visibility = calculateEnemyVisibility(
          projection.distance,
          atmosphere,
        );
      ...
      const hitStaggerX = getEnemyFlinchScreenOffset(
        projection.enemy,
        projection.size,
        time,
      );
      ...
      if (isRaycastEnemyTelegraphing(projection.enemy, time)) {
        ...
        const progress = getRaycastEnemySpawnTelegraphProgress(
          projection.enemy,
          time,
        );
        ...
        const markerRadius =
          projection.size * (0.42 + progress * 0.14 + pulse * 0.08);
        const haloRadius =
          projection.size * (0.72 + progress * 0.2 + pulse * 0.1);
        const alpha = (0.42 + pulse * 0.18) * visibility;
        this.graphics.fillStyle(
          RAYCAST_PALETTE.criticalVeil,
          0.52 * visibility,
        );
        ...
        this.graphics.fillStyle(
          spawnIdentity.telegraphColor,
          0.16 * visibility + progress * 0.08,
        );
        ...
        this.graphics.fillStyle(
          spawnIdentity.windupColor,
          (0.55 + pulse * 0.16) * visibility,
        );
        this.graphics.fillRect(
          tx - projection.size * 0.2,
          height * 0.5 - projection.size * 0.9,
          projection.size * 0.4,
          5,
        );
      ...
      const windupProgress = getRaycastEnemyWindupProgress(
        projection.enemy,
        time,
      );
      const pulse = isWindingUp ? Math.sin(time / 30) * 0.5 + 0.5 : 0;
      const telegraphMix = isWindingUp
        ? 0.3 + windupProgress * 0.4 + pulse * 0.18
        : 0;
      const color =
        projection.enemy.hitFlashUntil > time
          ? this.blendColors(0xfff5f0, projection.enemy.color, 0.48)
          : this.blendColors(
              projection.enemy.color,
              RAYCAST_PALETTE.telegraphRose,
              telegraphMix,
            );
      const enemyStyle = getRaycastEnemyVisualStyle(
        projection.enemy.kind,
        projection.enemy.color,
      );
      const accentColor =
        projection.enemy.variantAccentColor ?? enemyStyle.accentColor;
      ...
      const size =
        readability.size *
        (isWindingUp ? 1.04 + windupProgress * 0.08 + pulse * 0.04 : 1);
      ...
      const currentSpriteTextureKey = this.getEnemyTextureKey(
        projection.enemy.kind,
        currentSpriteState,
      );
      const willDrawEnemySprite =
        currentSpriteTextureKey !== null &&
        raycastTextureExists(this.scene, currentSpriteTextureKey);

      if (!willDrawEnemySprite) {
        this.graphics.fillStyle(
          enemyStyle.outlineColor,
          RAYCAST_ENEMY_BILLBOARD_READABILITY.outlineAlpha * visibility,
        );
        this.graphics.fillEllipse(
          sx,
          height * 0.5 + size * 0.08,
          size * 1.28,
          size * 1.42,
        );
      }
      ...
      if (isWindingUp) {
        this.graphics.fillStyle(
          enemyStyle.windupColor,
          (0.18 + windupProgress * 0.14 + pulse * 0.08) * visibility,
        );
        this.graphics.fillCircle(
          sx,
          height * 0.5,
          size * (0.46 + windupProgress * 0.08),
        );
      }

      this.graphics.fillStyle(
        color,
        RAYCAST_ENEMY_BILLBOARD_READABILITY.fillAlpha * visibility,
      );
      const savedSilhouetteSize = projection.size;
      projection.size = size;
      const didDrawSprite = this.drawEnemySpriteIfAvailable(
        projection,
        height,
        visibility,
        time,
      ...
      if (isWindingUp) {
        ...
        this.graphics.fillStyle(
          enemyStyle.windupColor,
          (0.5 + windupProgress * 0.2) * visibility,
        );
        this.graphics.fillRect(
          sx - size * 0.42,
          height * 0.5 - size * 0.92,
          size * 0.84,
          6,
        );
        this.graphics.lineStyle(
  ...
  renderBoss(
    ...
    for (let i = 0; i < profile.ringCount; i += 1) {
      ...
      const ringScale =
        1.04 + i * 0.14 + Math.sin(time * 0.002 + i * 0.8) * 0.02;
      this.graphics.lineStyle(
    ...
    for (let i = 0; i < 3; i += 1) {
      ...
      this.graphics.fillStyle(
        this.blendColors(profile.haloColor, 0x11151f, 0.45),
        (0.44 - i * 0.06) * visibility,
      );
    ...
    for (let i = 0; i < profile.particleCount; i += 1) {
      const t =
        (i / profile.particleCount) * Math.PI * 2 +
        time * (0.0014 + (i % 3) * 0.0002);
      const pr =
        size * (0.72 + (i % 4) * 0.05 + Math.sin(time * 0.003 + i) * 0.03);
      this.graphics.fillStyle(
        profile.haloColor,
        (0.1 + ((i + 2) % 5) * 0.03) * visibility,
      );
      this.graphics.fillCircle(
        cx + Math.cos(t) * pr,
        cy + Math.sin(t) * pr * 0.76,
        Math.max(2, size * 0.032),
      );
    }
    if (telegraph) {
  ...
  renderEnemyProjectiles(
    ...
    for (let i = 0; i < projectiles.length; i += 1) {
      ...
      if (
        this.fillProjectileProjection(
          player,
          projectile,
          width,
          height,
          this.ensureProjectileProjectionSlot(n),
        )
      ) {
        n += 1;
      }
  ...
  renderBillboards(
    ...
    for (let i = 0; i < billboards.length; i += 1) {
      ...
      if (
        this.fillBillboardProjection(
          player,
          billboard,
          width,
          height,
          this.ensureBillboardProjectionSlot(n),
        )
      ) {
        n += 1;
      }
    ...
    for (let j = 0; j < list.length; j += 1) {
      ...
      const haloColor = isOpenGate
        ? getBillboardColor("gate-open", true)
        : RAYCAST_ATMOSPHERE.pickupHalo;
      this.graphics.fillStyle(haloColor, isOpenGate ? 0.24 : 0.18);
      this.graphics.fillCircle(
        projection.screenX,
        height * 0.5,
        projection.size + (isOpenGate ? 9 : 6),
      );
      ...
      if (projection.billboard.label) {
        ...
        this.graphics.fillRect(
          projection.screenX - 24,
          height * 0.5 - projection.size - 18,
          48,
          12,
        );
        this.graphics.fillStyle(projection.billboard.color, 0.95);
        this.graphics.fillRect(
          projection.screenX - 18,
          height * 0.5 - projection.size - 12,
          36,
          2,
        );
      }
  ...
  renderWeaponOverlay(
    ...
    const kick = Phaser.Math.Clamp(
      Math.max(muzzleAlpha, viewFeel.recoilKick),
      0,
      1,
    );
    ...
    const hasWeaponSprite =
      weaponTextureKey !== null &&
      raycastTextureExists(this.scene, weaponTextureKey);
    ...
    if (hasWeaponSprite && weaponTextureKey) {
      ...
      this.weaponSprite
        ...
        .setPosition(
          cx,
          weapon === "PISTOL"
            ? height + 78 + recoilY + viewFeel.bobY
            : height + 18 + recoilY + viewFeel.bobY,
        )
    ...
    const trimColor =
      weapon === "SHOTGUN"
        ? RAYCAST_PALETTE.rustBright
        : weapon === "LAUNCHER"
          ? RAYCAST_PALETTE.plasmaBright
          : RAYCAST_PALETTE.muzzleWarm;
    ...
    if (weapon === "PISTOL") {
      ...
      this.graphics.fillGradientStyle(
        weaponColor,
        this.blendColors(weaponColor, deepShadow, 0.55),
        0x152028,
        0x152028,
        0.96,
      );
      ...
      this.graphics.fillStyle(
        this.blendColors(weaponColor, 0x1a2a38, 0.35),
        0.96,
      );
      this.graphics.fillRect(cx - 11, baseY - 112, 22, 40);
      this.graphics.fillStyle(
        this.blendColors(weaponColor, 0x0e141c, 0.5),
        0.92,
      );
      ...
      this.graphics.fillStyle(
        this.blendColors(weaponColor, 0x0a0604, 0.55),
        0.9,
      );
      ...
      this.graphics.fillStyle(
        this.blendColors(weaponColor, 0x2a1408, 0.4),
        0.92,
      );
      ...
      this.graphics.fillStyle(
        this.blendColors(weaponColor, 0x0a1218, 0.4),
        0.88,
      );
      ...
      this.graphics.lineStyle(
        2,
        this.blendColors(trimColor, 0x0a2030, 0.4),
        0.55,
      );
      ...
      for (let band = 0; band < 3; band += 1) {
        this.graphics.lineStyle(
          1,
          this.blendColors(trimColor, deepShadow, 0.5),
          0.2,
        );
        this.graphics.lineBetween(
          cx - 36,
    ...
    this.graphics.fillCircle(
      cx,
      flashTop - 12,
      weapon === "SHOTGUN" ? 38 : weapon === "LAUNCHER" ? 46 : 26,
    );
    this.graphics.fillStyle(
      trimColor,
      flashAlpha * (weapon === "LAUNCHER" ? 0.82 : 0.95),
    );
    this.graphics.fillTriangle(
      cx - flashW,
      flashTop,
    ...
    this.graphics.fillCircle(
      cx,
      flashTop - 16,
      weapon === "SHOTGUN" ? 30 : weapon === "LAUNCHER" ? 38 : 20,
    );
  ...
  private drawBackground(
    ...
    const activeZone = getRaycastZoneVisual(
      this.level.zones,
      player.x,
      player.y,
    );
    ...
    for (let y = 0; y < horizonY; y += 4) {
      ...
      const bandSample = sampleRaycastGroundBand(
        activeSurface,
        bandIndex,
        "ceiling",
      );
      ...
      if (
        groundStyle.ceilingPattern === "crossbars" &&
        (y + bandSample.crossbarOffset) % 28 === 0
      ) {
        this.graphics.fillStyle(
          zoneTheme.signalColor,
          (0.02 + (1 - t) * 0.028) * bandSample.accentAlpha,
        );
        this.graphics.fillRect(0, y, width, 2);
        this.graphics.fillStyle(
          zoneTheme.patternColor,
          (0.012 + (1 - t) * 0.02) * bandSample.accentAlpha,
        );
        this.graphics.fillRect(width * 0.18, y + 2, width * 0.64, 1);
      } else if (
        groundStyle.ceilingPattern === "void-noise" &&
        (y + Math.floor(player.x * 10)) % 22 === 0
      ) {
        const laneX = (bandSample.laneOffset * 9) % Math.max(16, width);
        this.graphics.fillStyle(
          zoneTheme.patternColor,
          (0.018 + (1 - t) * 0.025) * bandSample.accentAlpha,
        );
        this.graphics.fillRect(
          laneX,
          y,
          width * (0.34 + bandSample.scatter * 0.4),
          1,
        );
        if (bandSample.scatter > 0.58) {
          this.graphics.fillRect(
            Math.max(0, width - laneX - width * 0.22),
            y + 2,
            width * 0.22,
            1,
          );
        ...
        this.graphics.fillStyle(
          zoneTheme.patternColor,
          (0.012 + (1 - t) * 0.018) * bandSample.accentAlpha,
        );
        this.graphics.fillRect(
          width * 0.12,
          y,
          width * (0.2 + bandSample.scatter * 0.32),
          1,
        );
        this.graphics.fillRect(
          width * 0.62,
          y,
          width * (0.12 + bandSample.scatter * 0.18),
          1,
        );
      }
    }
...
this.graphics.fillTriangle(
  ...
  horizonY,
    ...
    for (let y = horizonY; y < height; y += 4) {
      ...
      const variant = getRaycastCellVariant(
        Math.floor(player.x * 3 + t * 19),
        Math.floor(player.y * 3 + t * 27),
      );
      ...
      const color = this.blendColors(
        this.blendColors(
          base,
          groundStyle.floorGlowColor,
          bandSample.scatter * 0.05,
        ),
      ...
      if (t < 0.1) {
        this.graphics.fillStyle(
          zoneTheme.accentColor,
          0.011 * (1 - t / 0.1) * bandSample.accentAlpha,
        );
        this.graphics.fillRect(0, y, width, 2);
      }

      if (
        groundStyle.floorPattern === "scanlines" &&
        (y + Math.floor(player.x * 12)) % 24 === 0
      ) {
        this.graphics.fillStyle(
          zoneTheme.patternColor,
          (0.024 + (1 - t) * 0.028) * bandSample.accentAlpha,
        );
        this.graphics.fillRect(0, y, width, 1);
        this.graphics.fillRect(
          width * 0.16 + bandSample.laneOffset,
          y + 2,
          width * 0.14,
          1,
        );
      } else if (
        groundStyle.floorPattern === "grid-cells" &&
        y % groundStyle.cellStride < 4
      ) {
        this.graphics.fillStyle(
          groundStyle.floorGlowColor,
          (0.02 + (1 - t) * 0.025) * bandSample.accentAlpha,
        );
        this.graphics.fillRect(0, y, width, 1);
        for (
          let x =
            (Math.floor(player.x * 14) + bandSample.laneOffset) %
            groundStyle.cellStride;
          x < width;
          x += groundStyle.cellStride
        ) {
          this.graphics.fillRect(x, y, 1, 4);
        }
      } else if (
        groundStyle.floorPattern === "hazard-lattice" &&
        y % groundStyle.cellStride < 4
      ) {
        this.graphics.fillStyle(
          groundStyle.floorGlowColor,
          (0.03 + (1 - t) * 0.036) * bandSample.accentAlpha,
        );
        this.graphics.fillRect(0, y, width, 2);
        for (
          let x =
            (Math.floor(player.y * 11) + bandSample.laneOffset) %
            groundStyle.cellStride;
          x < width;
          x += groundStyle.cellStride
        ) {
        ...
        groundStyle.floorPattern === "glow-rings" &&
        y % groundStyle.cellStride < 4
      ) {
        const ringInset = width * (0.24 + bandSample.scatter * 0.08);
        this.graphics.fillStyle(
          groundStyle.floorGlowColor,
          (0.028 + (1 - t) * 0.046) * bandSample.accentAlpha,
        );
        this.graphics.fillRect(ringInset, y, width - ringInset * 2, 2);
        this.graphics.fillRect(
          ringInset + bandSample.segmentLength,
          y + 2,
          Math.max(16, width * 0.18),
          1,
        );
      } else if (groundStyle.floorPattern === "noise-cells") {
        const cellStride = groundStyle.cellStride;
        const rowSeed = Math.floor(
          player.x * 9 + y * 0.15 + bandSample.laneOffset,
        );
        if (y % Math.max(8, Math.floor(cellStride * 0.66)) === 0) {
          for (
            let x = 0;
            x < width;
            x += Math.max(10, Math.floor(cellStride * 0.75))
          ) {
            const noise = getRaycastCellVariant(
              Math.floor(x * 0.2) + rowSeed,
              Math.floor(y * 0.18) + Math.floor(player.y * 8),
            );
            if (noise > 0.52) {
              this.graphics.fillStyle(
                groundStyle.floorGlowColor,
                (0.012 + noise * 0.04 * (1 - t)) * bandSample.accentAlpha,
              );
              this.graphics.fillRect(
                x,
                y,
                Math.max(3, cellStride * (0.24 + bandSample.scatter * 0.22)),
                2,
              );
            }
          }
      ...
      if (
        bandSample.scatter > 0.6 &&
        y % Math.max(12, Math.floor(groundStyle.cellStride * 0.55)) === 0
      ) {
        this.graphics.fillStyle(
          zoneTheme.patternColor,
          0.01 + (1 - t) * 0.018 * groundStyle.floorBandAlpha,
        );
        this.graphics.fillRect(
          width * 0.5 - bandSample.segmentLength,
          y + 1,
          bandSample.segmentLength * 2,
          1,
        );
      }
    }
...
this.graphics.fillTriangle(
  ...
  height,
    ...
    if (hzLm === "gate" || hzLm === "ambush" || hzLm === "reactor") {
      this.graphics.lineStyle(
        2,
        zoneTheme.signalColor,
        0.12 + atmosphere.pulseAlpha * 0.35,
      );
      this.graphics.lineBetween(
        width * 0.5 - 44,
        horizonY + 26,
    ...
    if (hzLm === "bridge") {
      this.graphics.lineStyle(
        2,
        zoneTheme.signalColor,
        0.09 + atmosphere.pulseAlpha * 0.22,
      );
      this.graphics.lineBetween(
        width * 0.08,
        horizonY + 22,
    ...
    if (hzLm === "core") {
      this.graphics.lineStyle(
        1,
        zoneTheme.signalColor,
        0.11 + atmosphere.pulseAlpha * 0.42,
      );
    ...
    this.graphics.fillStyle(
      zoneTheme.signalColor,
      0.028 + atmosphere.pulseAlpha * 0.04,
    );
    this.graphics.fillRect(0, horizonY - 1, width, 2);

    const vignette = this.blendColors(
      0x010206,
      RAYCAST_ATMOSPHERE.fogColor,
      0.55,
    );
    ...
    return RAYCAST_OPTIONAL_TEXTURE_KEYS.wall01;
  }

  private getWallTextureU(hit: RaycastHit): number {
    const fracX = hit.hitX - Math.floor(hit.hitX);
    const fracY = hit.hitY - Math.floor(hit.hitY);

    const edgeX = Math.abs(fracX - 0.5);
    const edgeY = Math.abs(fracY - 0.5);

    return edgeX > edgeY ? fracY : fracX;
  }

  private getWallTextureRegion(textureKey: string): {
  u0: number;
  u1: number;
  v0: number;
  v1: number;
} {
  if (textureKey === RAYCAST_OPTIONAL_TEXTURE_KEYS.wall01) {
    return {
      u0: 0.34,
      u1: 0.66,
      v0: 0.24,
  ...
  if (textureKey === RAYCAST_OPTIONAL_TEXTURE_KEYS.wall02) {
    return {
      u0: 0.32,
      u1: 0.68,
      v0: 0.12,
  ...
  return {
    ...
    private getWallTextureSample(textureKey: string): RaycastWallTextureSample | null {
    const cachedSample = this.wallTextureSamples.get(textureKey);

    if (cachedSample) {
      return cachedSample;
    }

    if (!raycastTextureExists(this.scene, textureKey)) {
      return null;
    }

    const texture = this.scene.textures.get(textureKey);
    const sourceImage = texture.getSourceImage() as HTMLImageElement | HTMLCanvasElement | null;
    ...
    const sample = { width, height, pixels };

    this.wallTextureSamples.set(textureKey, sample);

    return sample;
  }

private sampleWallTextureBilinear(
  sample: RaycastWallTextureSample,
  u: number,
  v: number,
): number {
  const tx = Phaser.Math.Clamp(u, 0, 1) * (sample.width - 1);
  const ty = Phaser.Math.Clamp(v, 0, 1) * (sample.height - 1);

  const x0 = Math.floor(tx);
  const y0 = Math.floor(ty);
  const x1 = Math.min(x0 + 1, sample.width - 1);
  const y1 = Math.min(y0 + 1, sample.height - 1);

  const fx = tx - x0;
  const fy = ty - y0;

  const i00 = (y0 * sample.width + x0) * 4;
  const i10 = (y0 * sample.width + x1) * 4;
  const i01 = (y1 * sample.width + x0) * 4;
  const i11 = (y1 * sample.width + x1) * 4;

  const r =
    (sample.pixels[i00] ?? 0) * (1 - fx) * (1 - fy) +
    (sample.pixels[i10] ?? 0) * fx * (1 - fy) +
    (sample.pixels[i01] ?? 0) * (1 - fx) * fy +
    (sample.pixels[i11] ?? 0) * fx * fy;

  const g =
    (sample.pixels[i00 + 1] ?? 0) * (1 - fx) * (1 - fy) +
    (sample.pixels[i10 + 1] ?? 0) * fx * (1 - fy) +
    (sample.pixels[i01 + 1] ?? 0) * (1 - fx) * fy +
    (sample.pixels[i11 + 1] ?? 0) * fx * fy;

  const b =
    (sample.pixels[i00 + 2] ?? 0) * (1 - fx) * (1 - fy) +
    (sample.pixels[i10 + 2] ?? 0) * fx * (1 - fy) +
    (sample.pixels[i01 + 2] ?? 0) * (1 - fx) * fy +
    (sample.pixels[i11 + 2] ?? 0) * fx * fy;

  return (Math.round(r) << 16) | (Math.round(g) << 8) | Math.round(b);
}

private sampleWallTextureColor(
  sample: RaycastWallTextureSample,
  u: number,
  v: number,
  shade: number,
  ...
  const baseColor = this.sampleWallTextureBilinear(sample, u, v);

  const readableShade = Phaser.Math.Clamp(shade + 0.18, 0.52, 1);
  const shadedColor = this.applyShade(baseColor, readableShade);
  const fogMix = Phaser.Math.Clamp((1 - readableShade) * 0.18, 0, 0.18);

  return this.blendColors(shadedColor, atmosphere.fogColor, fogMix);
}

  private drawTexturedWallColumn(
    hit: RaycastHit,
    x: number,
    y: number,
    width: number,
    ...
    const textureKey = this.getWallTextureKey(hit, surface);

    if (!textureKey) return false;

    const sample = this.getWallTextureSample(textureKey);

    if (!sample) return false;

    const u = this.getWallTextureU(hit);

    const bands = Phaser.Math.Clamp(Math.floor(wallHeight), 64, 320);
    const bandHeight = wallHeight / bands;

    for (let band = 0; band < bands; band += 1) {
      const v = (band + 0.5) / bands;
      const color = this.sampleWallTextureColor(
        sample,
        u,
        v,
        shade,
  ...
  private drawWallColumnVolume(
    ...
    this.graphics.fillStyle(
      midGlow,
      0.038 * shade * (0.82 + surface.variant * 0.35),
    );
    ...
    const speck = getRaycastCellVariant(
      column + surface.cellX * 3,
      surface.cellY * 5,
    );
  ...
  private drawWallPattern(
    ...
    const patternColor = this.blendColors(
      this.applyShade(
        this.blendColors(
          RAYCAST_ATMOSPHERE.wallPatternColors[
            wallType as keyof typeof RAYCAST_ATMOSPHERE.wallPatternColors
          ] ?? 0x5f7190,
    ...
    const alpha = Phaser.Math.Clamp(
      (shade - atmosphere.ambientDarkness) * (0.42 + surface.variant * 0.18),
      0.05,
      0.32,
    );
    ...
    const insetWidth = Math.max(
      1,
      width - width * detail.horizontalInset * 0.4,
    );
    ...
    if (
      wallStyle.pattern === "terminal-panels" &&
      column % wallStyle.panelStride === 0
    ) {
      this.graphics.fillStyle(patternColor, alpha);
      this.graphics.fillRect(
        insetX,
        y + wallHeight * (0.18 + detail.bandOffset),
        insetWidth,
        Math.max(2, wallHeight * 0.028),
      );
      this.graphics.fillRect(
        insetX,
        y + wallHeight * (0.64 - detail.bandOffset * 0.5),
        insetWidth,
        Math.max(2, wallHeight * 0.022),
      );
      ...
      this.graphics.fillRect(
        ...
        y + wallHeight * 0.34,
        insetWidth,
        Math.max(3, wallHeight * (0.1 + detail.chip * 0.05)),
      );
      if (detail.energy > 0.62) {
        ...
        this.graphics.fillRect(
          x + width * 0.5,
          y + wallHeight * 0.22,
          Math.max(1, width * 0.35),
          Math.max(2, wallHeight * 0.12),
        );
      }
    } else if (
      wallStyle.pattern === "corrupted-ribs" &&
      column % wallStyle.panelStride === 0
    ) {
      ...
      if (detail.diagonalFlip) {
        this.graphics.lineBetween(
          x,
          y + wallHeight * (0.18 + detail.seamOffset * 0.2),
          x + width,
          y + wallHeight * 0.42,
        );
        this.graphics.lineBetween(
          ...
          y + wallHeight * 0.42,
          x,
          y + wallHeight * (0.76 - detail.seamOffset * 0.2),
        );
      } else {
        this.graphics.lineBetween(
          x + width,
      ...
      this.graphics.fillRect(
        insetX,
        y + wallHeight * (0.48 + detail.bandOffset * 0.6),
        insetWidth,
        Math.max(2, wallHeight * 0.06),
      );
    } else if (
      wallStyle.pattern === "hazard-strips" &&
      column % Math.max(6, wallStyle.panelStride - 2) === 0
    ) {
      this.graphics.fillStyle(patternColor, alpha + pulseAlpha);
      this.graphics.fillRect(
        insetX,
        y + wallHeight * (0.16 + detail.bandOffset * 0.5),
        insetWidth,
        Math.max(2, wallHeight * 0.1),
      );
      this.graphics.fillRect(
        insetX,
        y + wallHeight * (0.56 - detail.bandOffset * 0.35),
        insetWidth,
        Math.max(2, wallHeight * 0.08),
      );
      this.graphics.lineStyle(1, signalColor, alpha + 0.12 + pulseAlpha);
      this.graphics.lineBetween(
        x,
        y + wallHeight * (0.26 + detail.seamOffset * 0.12),
        x + width,
        y + wallHeight * (0.34 - detail.seamOffset * 0.12),
      );
    } else if (
      wallStyle.pattern === "locked-warning-frame" &&
      column % wallStyle.panelStride === 0
    ) {
      this.graphics.lineStyle(1, signalColor, alpha + 0.14 + pulseAlpha);
      this.graphics.strokeRect(
        insetX,
        y + wallHeight * (0.22 + detail.bandOffset * 0.3),
        Math.max(2, insetWidth),
        Math.max(6, wallHeight * 0.5),
      );
      this.graphics.fillStyle(secondaryColor, alpha * 0.6);
      this.graphics.fillRect(
        insetX,
        y + wallHeight * (0.42 + detail.seamOffset * 0.08),
        insetWidth,
        Math.max(2, wallHeight * 0.06),
      );
    } else if (
      wallStyle.pattern === "exit-glow" &&
      column % wallStyle.panelStride === 0
    ) {
      this.graphics.fillStyle(signalColor, alpha * 0.5 + pulseAlpha);
      this.graphics.fillRect(
        insetX,
        y + wallHeight * (0.14 + detail.bandOffset * 0.3),
        insetWidth,
        Math.max(3, wallHeight * 0.14),
      );
      this.graphics.lineStyle(2, patternColor, alpha + 0.16);
      this.graphics.strokeCircle(
        x + width * 0.5,
        y + wallHeight * 0.34,
        Math.max(2, width * 0.72),
      );
    } else if (
      wallStyle.pattern === "data-noise-cells" &&
      column % wallStyle.panelStride === 0
    ) {
      const noiseStep = wallHeight * 0.1;
      for (let row = 0; row < 4; row += 1) {
        const noise = getRaycastCellVariant(
          surface.cellX + column + row,
          surface.cellY + row,
        );
        const rowY =
          y + wallHeight * (0.18 + row * 0.16 + detail.bandOffset * 0.35);
        if (noise > 0.42) {
          this.graphics.fillStyle(
            noise > 0.72 ? signalColor : patternColor,
            alpha * (0.6 + noise * 0.2),
          );
          this.graphics.fillRect(
            insetX,
            rowY,
            insetWidth,
            Math.max(2, noiseStep * (0.18 + noise * 0.24)),
          );
        }
      }
    }
    ...
    if (
      detail.chip > 0.66 &&
      column % Math.max(5, wallStyle.panelStride - 4) === 0
    ) {
      this.graphics.fillStyle(secondaryColor, alpha * 0.4);
      this.graphics.fillRect(
        x,
        y + wallHeight * (0.28 + detail.seamOffset * 0.15),
        Math.max(1, width * 0.35),
        Math.max(2, wallHeight * 0.05),
      );
    }
    ...
    if (surface.landmark === "ambush" && column % 19 === 0) {
      this.graphics.lineStyle(
        2,
        signalColor,
        alpha + atmosphere.pulseAlpha * 0.6,
      );
      this.graphics.lineBetween(
        x,
        y + wallHeight * 0.24,
  ...
  private drawEnemyDeathBurst(
    ...
    const burstSize =
      projection.size * scaleMul * (1.28 + (1 - alpha) * 0.92);
    const shard = 1 - alpha;
    const cx = projection.screenX;
    const cy = height * 0.5;
    ...
    this.graphics.fillStyle(
      0xfff8f0,
      alpha * 0.34 * visibility * (0.25 + shard * 0.75),
    );
    this.graphics.fillCircle(cx, cy, burstSize * 0.24);
    this.graphics.fillStyle(
      RAYCAST_PALETTE.telegraphRose,
      alpha * 0.28 * visibility * shard,
    );
    ...
    for (let i = 0; i < shards; i += 1) {
      ...
      this.graphics.lineStyle(
        2,
        i % 2 === 0 ? 0xfff0d0 : RAYCAST_PALETTE.plasmaBright,
        alpha * 0.64 * visibility * shard,
      );
      this.graphics.lineBetween(x0, y0, x1, y1);
      this.graphics.fillStyle(
        projection.enemy.color,
        alpha * 0.5 * visibility * shard,
      );
  ...
  private getEnemySpriteState(
    projection: EnemyProjection,
    time: number,
  ): RaycastEnemySpriteState {
  ...
  private getEnemySpriteFromPool(): Phaser.GameObjects.Image {
    ...
    if (!this.enemySpritePool[index]) {
      const image = this.scene.add.image(
        0,
        0,
        RAYCAST_ENEMY_TEXTURE_KEYS.GRUNT.idle,
      );
  ...
  private hideUnusedEnemySprites(): void {
    for (
      let i = this.activeEnemySpriteCount;
      i < this.enemySpritePool.length;
      i += 1
    ) {
  ...
  private drawEnemySilhouette(
    ...
    const accentColor = this.blendColors(
      style.accentColor,
      RAYCAST_PALETTE.telegraphRose,
      telegraphMix * 0.45,
    );
    ...
    if (style.silhouette === "juggernaut") {
      ...
      this.graphics.fillEllipse(
        ...
        projection.size * 0.54,
        projection.size * 0.14,
      );
      this.drawEnemyHeadAccent(
        projection.screenX,
      ...
      this.graphics.fillCircle(
        projection.screenX,
        bodyTop + projection.size * 0.56,
        projection.size * 0.11,
      );
      this.graphics.fillStyle(style.eyeColor, 0.98 * visibility);
      this.graphics.fillCircle(
        bodyLeft + projection.size * 0.42,
        bodyTop + projection.size * 0.18,
        projection.size * 0.04,
      );
      this.graphics.fillCircle(
        bodyLeft + projection.size * 0.58,
        bodyTop + projection.size * 0.18,
        projection.size * 0.04,
      );
      ...
      this.graphics.fillRect(
        ...
        bodyTop + projection.size * 0.28,
        projection.size * 0.2,
        projection.size * 0.5,
      );
      this.graphics.fillRect(
        bodyLeft + projection.size * 0.72,
        bodyTop + projection.size * 0.28,
        projection.size * 0.2,
        projection.size * 0.5,
      );
      for (const rx of [0.22, 0.5, 0.78] as const) {
        ...
        this.graphics.fillCircle(
          bodyLeft + projection.size * rx,
          bodyTop + projection.size * 0.4,
          Math.max(1.2, projection.size * 0.04),
        );
      }
      this.graphics.fillStyle(0x120805, 0.45 * visibility);
      this.graphics.fillEllipse(
        bodyLeft + projection.size * 0.22,
    ...
    if (style.silhouette === "phantom") {
      ...
      this.graphics.lineBetween(
        ...
        bodyTop + projection.size * 0.16,
        projection.screenX,
        bodyTop + projection.size * 0.86,
      );
      this.graphics.lineBetween(
        projection.screenX - projection.size * 0.16,
        centerY + projection.size * 0.1,
        projection.screenX - projection.size * 0.3,
        centerY + projection.size * 0.42,
      );
      this.graphics.lineBetween(
        projection.screenX + projection.size * 0.16,
        centerY + projection.size * 0.1,
      ...
      this.graphics.fillCircle(
        projection.screenX - projection.size * 0.05,
        bodyTop + projection.size * 0.22,
        projection.size * 0.03,
      );
      this.graphics.fillCircle(
        projection.screenX + projection.size * 0.05,
        bodyTop + projection.size * 0.22,
      ...
      this.graphics.lineStyle(
        1,
        this.blendColors(accentColor, 0x0a1a14, 0.45),
        0.22 * visibility,
      );
      for (let w = 0; w < 4; w += 1) {
        const o = w * 0.14;
    ...
    if (style.silhouette === "sentinel") {
      ...
      this.graphics.fillCircle(
        projection.screenX,
        bodyTop + projection.size * 0.18,
        projection.size * 0.17,
      );
      this.graphics.fillEllipse(
        projection.screenX,
        bodyTop + projection.size * 0.54,
        projection.size * 0.64,
        projection.size * 0.72,
      );
      this.graphics.fillRect(
      ...
      this.graphics.fillCircle(
        bodyLeft + projection.size * 0.16,
        centerY,
        projection.size * 0.11,
      );
      this.graphics.fillCircle(
        bodyLeft + projection.size * 0.84,
        centerY,
        projection.size * 0.11,
      );
      this.graphics.fillTriangle(
      ...
      this.graphics.lineBetween(
        bodyLeft + projection.size * 0.4,
        bodyTop + projection.size * 0.02,
        bodyLeft + projection.size * 0.34,
        bodyTop - projection.size * 0.16,
      );
      this.graphics.lineBetween(
        bodyLeft + projection.size * 0.6,
        bodyTop + projection.size * 0.02,
        bodyLeft + projection.size * 0.66,
        bodyTop - projection.size * 0.16,
      );
      ...
      this.graphics.fillStyle(
        this.blendColors(style.coreColor, 0x1a2a30, 0.5),
        0.85 * visibility,
      );
      this.graphics.fillRect(
        bodyLeft + projection.size * 0.82,
    ...
    this.graphics.fillEllipse(
      ...
      bodyTop + projection.size * 0.52,
      projection.size * 0.52,
      projection.size * 0.44,
    );
    this.graphics.fillStyle(color, 0.95 * visibility);
    this.graphics.fillCircle(
      projection.screenX,
      bodyTop + projection.size * 0.17,
      projection.size * 0.13,
    );
    ...
    this.graphics.fillRect(
      bodyLeft + projection.size * 0.34,
      bodyTop + projection.size * 0.19,
      projection.size * 0.32,
      projection.size * 0.05,
    );
    this.graphics.fillStyle(style.eyeColor, 0.94 * visibility);
    this.graphics.fillRect(
      bodyLeft + projection.size * 0.36,
      bodyTop + projection.size * 0.2,
      projection.size * 0.1,
      projection.size * 0.03,
    );
    this.graphics.fillRect(
      bodyLeft + projection.size * 0.54,
      bodyTop + projection.size * 0.2,
      projection.size * 0.1,
      projection.size * 0.03,
    );
  ...
  private drawBillboardGlyph(
    ...
    if (projection.billboard.style === "gate") {
      ...
      this.graphics.lineBetween(
        projection.screenX - projection.size * 0.52,
        y - projection.size * 0.74,
        projection.screenX - projection.size * 0.52,
        y + projection.size * 0.74,
      );
      this.graphics.lineBetween(
      ...
      this.graphics.lineBetween(
        projection.screenX - projection.size * 0.22,
        y - projection.size * 0.74,
        projection.screenX - projection.size * 0.22,
        y + projection.size * 0.74,
      );
    ...
    if (projection.billboard.style === "gate-open") {
      ...
      this.graphics.lineBetween(
        projection.screenX - projection.size * 0.68,
        y - projection.size * 0.58,
        projection.screenX - projection.size * 0.22,
        y,
      );
      this.graphics.lineBetween(
    ...
    if (projection.billboard.style === "health") {
      ...
      this.graphics.lineBetween(
        projection.screenX - projection.size * 0.24,
        y,
        projection.screenX + projection.size * 0.24,
        y,
      );
  ...
  private blendColors(
    ...
    const r =
      ((baseColor >> 16) & 0xff) * inverse +
      ((blendColor >> 16) & 0xff) * clampedAmount;
    const g =
      ((baseColor >> 8) & 0xff) * inverse +
      ((blendColor >> 8) & 0xff) * clampedAmount;
    const b =
      (baseColor & 0xff) * inverse + (blendColor & 0xff) * clampedAmount;
    return (Math.floor(r) << 16) + (Math.floor(g) << 8) + Math.floor(b);
  }
