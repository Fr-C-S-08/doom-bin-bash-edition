...
Presenter-facing guide for portfolio reviews, interviews, and internal playtests. **Target length:** **3–5 minutes** (tables below default to ~4 min). For a **minimal ordered checklist** (menú → … → World 2 teaser), use **[demo-script.md](./demo-script.md)**. Primary mode is **`RaycastScene`** (Episode 1 + boss + optional later worlds). `ArenaScene` is legacy test code — **not** in the current menu boot list.
...
|------|--------|----------------|
| 0:00–0:45 | **Menu** | Primary mode is **A / 3D**; **D** is difficulty; optional **Settings** for input. |
...
| 8–10 min | Clear or finale overlay; **ESC** menu; optional **Settings** / gamepad note |
...
- **Arena:** Legacy code only (not menu-booted); raycast is the product story.
